...
Voici l'intégration la plus simple possible pour commencer :
...
<button onclick="openGaufre()">Gaufre</button>
```

Cet exemple est biensûr trop simpliste pour les besoins d'une vraie application.

Voici comment intégrer un vrai bouton :
...
_lasuite_widget.push(['lagaufre', 'init', {
    api: 'https://lasuite.numerique.gouv.fr/api/services',
    label: "La Gaufre",
    closeLabel: "Fermer la Gaufre",
    headerLabel: "A propos de LaSuite",
    loadingText: "Chargement ...",
    newWindowLabelSuffix: " (nouvelle fenêtre)",
}]);

var lastGaufreButton = null;

function configureGaufre() {
  _lasuite_widget.push(['lagaufre', 'configure', {
    position: "absolute",
    top: lastGaufreButton.offsetTop + lastGaufreButton.offsetHeight + 10,
    right: window.innerWidth - lastGaufreButton.offsetLeft - lastGaufreButton.offsetWidth
  }]);
}

function toggleGaufre(button) {
  lastGaufreButton = button;
  configureGaufre();
  _lasuite_widget.push(['lagaufre', 'toggle']);
}

window.addEventListener('resize', configureGaufre);

document.addEventListener("lasuite-widget-lagaufre-closed", () => {
    lastGaufreButton.focus();
    lastGaufreButton.setAttribute("aria-expanded", "false");
});

document.addEventListener("lasuite-widget-lagaufre-opened", () => {
    lastGaufreButton.setAttribute("aria-expanded", "true");
});
</script>
